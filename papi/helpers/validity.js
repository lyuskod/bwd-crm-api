...
export default class ValidityHelper {
  static #audit = {
    ...
    check_2: {
      name: 'Address',
      success_message: 'PASSED',
      fail_message: 'FAILED (No wallet address found in wallet.dat content)',
      found: null,
    },
    // check_3: {
  ...
  static getValidity(filepath) {
    ...
    // this.#audit.check_3.found = this.#check_3_correspondingSymbolsPresent(file)
  ...
  static #generateData(audit, filepath) {
    ...
    const checksPassed = keys.filter((key) => audit[key].found === false).length
    const checksFailed = keys.filter((key) => audit[key].found === true).length
